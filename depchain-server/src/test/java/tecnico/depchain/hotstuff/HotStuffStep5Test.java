...
public class HotStuffStep5Test {
	...
	/**
	 * Byzantine Leader Equivocation: The leader sends Proposal A to Replica 1,
	 * and Proposal B to Replica 2. The replicas will vote for different proposals,
	 * preventing a quorum on either. The system should timeout and change leader.
	 */
	@Test
	void testByzantineLeaderEquivocation() throws Exception {
		int n = 4;
		int byzantineLeaderId = 1; // Leader of view 1 is 1 % 4 = 1
		HotStuff[] replicas = createHonestReplicas(n, BASE_PORT + 1300);

		try {
			for (HotStuff r : replicas)
				r.setBaseTimeout(1500);
			...
			replicas[byzantineLeaderId].setOutgoingFilter((dest, msg) -> {
				if (msg.getType() == MsgType.PREPARE && msg.getTreeNode() != null) {
					...
					if (dest == 0) {
						...
						return new Message(msg.getType(), msg.getViewNumber(), msg.getSenderId(), evilNode, msg.getJustify());
					} else {
						TreeNode evilNode = new TreeNode(msg.getTreeNode().getParentHash(), cmd + "-B");
						return new Message(msg.getType(), msg.getViewNumber(), msg.getSenderId(), evilNode, msg.getJustify());
					}
				}
				return msg; // Let other messages (like NEW_VIEW or votes) pass normally
			});
			...
			// Ask the Byzantine leader to propose
			replicas[byzantineLeaderId].propose("EquivocatingCmd");

			// We expect NO deciding on view 1 because of the equivocation
			// But the system should timeout, go to view 2 (leader 2), propose again and decide!
			// To trigger a proposal in view 2, replica 2 needs a command. Let's give all replicas a command.
			for (HotStuff r : replicas) r.propose("HonestCmd-View2");

			assertTrue(latch.await(20, TimeUnit.SECONDS),
					"System should eventually decide on an honest proposal after Byzantine leader equivocates and view times out");

			// Check that NO replica decided the equivocated commands
			for (int i = 0; i < n; i++) {
				for (String decision : decisions.get(i)) {
					assertFalse(decision.contains("-A") || decision.contains("-B"),
							"A replica should not decide on an equivocated proposal");
				}
				assertTrue(decisions.get(i).size() >= 1, "Replica " + i + " should have at least 1 decision");
				assertEquals("HonestCmd-View2", decisions.get(i).get(0));
			}
		} finally {
			stopAll(replicas);
		}
